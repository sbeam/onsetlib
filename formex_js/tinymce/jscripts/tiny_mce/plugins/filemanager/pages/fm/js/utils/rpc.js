/**
 * $Id: rpc.js 533 2008-10-29 19:40:22Z spocke $
 *
 * @author Moxiecode
...
(function($){
	window.RPC = {
		...
		insertFiles : function(s) {
			...
			$(s.selected_paths).each(function(i, v) {
				args['path' + i] = v; 
			});
			...
			RPC.exec('fm.insertFiles', args,  function (data) {
				...
				$(RPC.toArray(data.result)).each(function(i, v) {
					...
					if (v.path == s.focused_path)
						o.focusedFile = v;

					o.selectedFiles.push(v);
				});

				if (s.oninsert) {
					if (s.insert_filter) {
						s.insert_filter(o.focusedFile);

						$(o.selectedFiles).each(function(i, f) {
							s.insert_filter(f);
						});
					}
		...
		insertFile : function(s) {
			...
			RPC.exec('fm.insertFiles', {path0 : s.path},  function(data) {
				var o = {};
