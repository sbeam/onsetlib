(function($){
	window.UploadDialog = {
		...
		init : function() {
			...
			$('.uploadtype').html($.translate('{#upload.basic_upload}', 0, {a : '<a id="singleupload" href="#">', '/a' : '</a>'}));
			$('#content').show();
			...
			RPC.exec('fm.getConfig', {path : args.path}, function(data) {
				...
				t.chunkSize = config['upload.chunk_size'] || 1024 * 1024;
		...
		insertFiles : function(pa) {
			...
			if (s.onupload) {
				RPC.insertFiles({
					...
					oninsert : function(o) {
						if (o.selectedFiles.length > 0)
							s.onupload(o.selectedFiles);
					}
				});
			}
		},
